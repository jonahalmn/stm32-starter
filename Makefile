...
load-mac: $(ELF)
	@echo "LOAD WITH CUBE PROGRAMMER"
	$(CUBEPROGRAMMER_PATH)$(BINPROGRAMMER_PATH)/STM32_Programmer_CLI -c port=SWD -w bin/main.elf 0x80000000 --start
...
device-list:tmp-folder
	@-find /dev -name '*usb*' -depth 1 1> tmp/device_list.txt 2> /dev/null
	@cat tmp/device_list.txt
